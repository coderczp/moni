...
public interface MemMapSorter {
	...
	class HeapSorter implements MemMapSorter {
		...
		private void heapify(MemMapBytesArray arr, int idx, int max,
				Comparator<byte[]> cmp) {
			int index = 2 * idx;
	...
	class MergeSorter implements MemMapSorter {
		...
		private void merge(MemMapBytesArray arr, int beg, int mid, int end,
				Comparator<byte[]> cmp) {
			int i = beg;
			int j = mid + 1;
			while (i < j && j <= end) {
		...
		private void mergeSort(MemMapBytesArray arr, int beg, int end,
				Comparator<byte[]> cmp) {
			if (beg < end) {
				int mid = (beg + end) / 2;
